 ...
 // States for Editing GL Code
  ...
  const fetchGLHistoryForSelectedDate = async () => {
    ...
    try {
      ...
      // const dateString = glSelectedDate.toISOString().split('T')[0]; // Format: YYYY-MM-DD
      // const response = await fetch(`${API_BASE_URL}/api/v1/company/financial/gl-history?date=${dateString}`, {
      //   method: 'GET',
      //   headers: { 'Authorization': `Bearer ${authToken}`, 'Accept': 'application/json' }
      // });
      // const responseData = await response.json();
      // if (!response.ok) {
      //   throw new Error(responseData.detail || `Failed to fetch GL history. Status: ${response.status}`);
      // }
      // setGLHistoryData(responseData.history_records || []); // Adjust based on your API response structure
      // showGLToast(`History for ${glSelectedDate.toLocaleDateString()} fetched.`, 'success');
      // if ((responseData.history_records || []).length === 0) {
      ...
      {[
        ...
        <button
          ...
          onClick={() => {
            ...
            // Optionally reset states specific to other GL tabs if needed
            ...
            if (tab.id !== 'allGLCodes' && allGLCodesData) { /* Do not clear allGLCodesData here, let fetch be explicit */ }
          }}
        >
          <tab.icon className="mr-1.5 sm:mr-2 h-4 w-4 sm:h-5 sm:w-5" /> {tab.label}
                ...
                <div className="flex flex-col">
                  <div className='flex flex-col lg:flex-row items-start gap-6 mb-4'>
                    <div className="flex-shrink-0">
                        <img src={glUploadIllustration} className={glIllustrationCommonStyle} alt="Manual data entry illustration" />
                    </div>
                    <div className="flex-grow w-full space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div>
                          <label className="block mb-1 text-sm font-medium text-gray-700">GL Code</label>
                          <div className="relative">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500 text-sm">#</span>
                            <input type="number" name="general_ledger_code" value={glFormData.general_ledger_code} onChange={handleGLInputChange}
                              className="bg-sky-50 border border-sky-200 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full pl-8 p-2.5" placeholder="1001"/>
                          </div>
                        </div>
                        <div>
                          <label className="block mb-1 text-sm font-medium text-gray-700">Account Name</label>
                          <div className="relative">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><FiFile className="w-4 h-4 text-gray-400"/></span>
                            <input type="text" name="account_name" value={glFormData.account_name} onChange={handleGLInputChange}
                              className="bg-sky-50 border border-sky-200 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full pl-10 p-2.5" placeholder="Cash Account"/>
                          </div>
                        </div>
                        <div>
                          <label className="block mb-1 text-sm font-medium text-gray-700">Category</label>
                          <div className="relative">
                            <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><FiActivity className="w-4 h-4 text-gray-400"/></span>
                            <input type="text" name="category" value={glFormData.category} onChange={handleGLInputChange}
                              className="bg-sky-50 border border-sky-200 text-gray-900 text-sm rounded-lg focus:ring-sky-500 focus:border-sky-500 block w-full pl-10 p-2.5" placeholder="Assets"/>
                          </div>
                        </div>
                      </div>
                       <div className="flex justify-end">
                        <button type="button" onClick={handleGLAddEntry} className="bg-sky-600 hover:bg-sky-700 text-white px-5 py-2.5 rounded-lg flex items-center text-sm">
                  ...
                  {glEntries.length > 0 && (
                    <div className="mb-6">
                      <div className="flex justify-between items-center mb-3">
                        <h3 className="font-medium text-gray-700">Entries Preview ({glEntries.length})</h3>
                        <span className="text-sm text-sky-600 bg-sky-100 px-3 py-1 rounded-full">{glEntries.length} {glEntries.length === 1 ? 'entry' : 'entries'}</span>
                      </div>
                      <div className="overflow-x-auto rounded-lg border border-sky-100 max-h-80">
                        <table className="min-w-full divide-y divide-sky-100">
                          <thead className="bg-sky-50 sticky top-0"><tr>
                            <th className="px-4 py-3 text-left text-xs font-medium text-sky-600 uppercase">GL Code</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-sky-600 uppercase">Account Name</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-sky-600 uppercase">Category</th>
                          </tr></thead>
                          <tbody className="bg-white divide-y divide-sky-100">
                            {glEntries.map((entry, index) => (
                              <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-sky-50/70'}>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-800">{entry.general_ledger_code}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{entry.account_name}</td>
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{entry.category}</td>
                              </tr>))}
                          </tbody>
                        </table>
                      </div>
                  ...
                  <div className="flex justify-end mt-auto pt-4 border-t border-gray-200">
                    <button type="button" onClick={submitGLManualEntries} disabled={isGLLoading || glEntries.length === 0}
                      className={`px-6 py-3 rounded-lg flex items-center text-sm ${isGLLoading || glEntries.length === 0 ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 text-white'}`}>
                      {isGLLoading ? (<><TailSpin color="#FFFFFF" height={18} width={18} className="mr-2" /> Uploading...</>) : (<><FiUpload className="mr-2" /> Upload {glEntries.length} {glEntries.length === 1 ? 'Entry' : 'Entries'}</>)}
                    </button>
                  </div>
                </div>
            )}
            {glActiveTab === 'upload' && (
                <div>
                  <div className='flex flex-col lg:flex-row items-start gap-6 mb-4'>
                     <div className="flex-shrink-0">
                        <img src={glUploadIllustration} className={glIllustrationCommonStyle} alt="File upload illustration" />
                    </div>
                    <div className="flex-grow w-full">
                      <label className="block mb-2 text-sm font-medium text-gray-700">Upload Excel File</label>
                      <div
                        className={`flex items-center justify-center w-full h-48 border-2 border-dashed rounded-xl cursor-pointer ${isGLDragging ? 'border-sky-400 bg-sky-50' : 'border-sky-200 hover:border-sky-300 bg-sky-50 hover:bg-sky-100'}`}
                        onDragOver={handleGLDragOver} onDragLeave={handleGLDragLeave} onDrop={handleGLDrop}
                      >
                        <label htmlFor="gl-file-picker-upload-tab" className="flex flex-col items-center justify-center w-full h-full cursor-pointer">
                          <div className="flex flex-col items-center justify-center pt-5 pb-6 px-4 text-center">
                            ...
                            <p className="text-xs text-gray-500">Excel (.xls, .xlsx). Headers: general_ledger_code, account_name, category</p>
                            <input id="gl-file-picker-upload-tab" type="file" className="hidden" accept=".xlsx,.xls,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={handleGLFilePickerChange} disabled={isGLLoading && glFile != null} />
                          </div>
                        </label>
                      </div>
                    </div>
                  ...
                  {isGLLoading && glFile && !glParsedFileEntries.length && (
                    <div className="text-center py-4">
                        <TailSpin color="#0EA5E9" height={30} width={30} />
                        <p className="text-sm text-sky-600 mt-2">Parsing file: {glFile.name}...</p>
                    </div>
                  )}

                  {glParsedFileEntries.length > 0 && (
                    <div className="mb-6">
                      <div className="flex justify-between items-center mb-3">
                        <div>
                            <h3 className="font-medium text-gray-700">Parsed Entries from {glFile?.name} ({glParsedFileEntries.length})</h3>
                            <p className="text-xs text-gray-500">Review and edit data before uploading.</p>
                        </div>
                        <button onClick={removeGLFileAndParsedData} className="text-sm text-red-500 hover:text-red-700 flex items-center" disabled={isGLLoading}>
                          <FiX className="mr-1" /> Clear Parsed Data
                        </button>
                      </div>
                      <div className="overflow-x-auto rounded-lg border border-sky-100 max-h-96">
                        <table className="min-w-full divide-y divide-sky-100">
                          <thead className="bg-sky-50 sticky top-0">
                            <tr>
                              <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">GL Code</th>
                              <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">Account Name</th>
                              <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">Category</th>
                            </tr>
                          </thead>
                          <tbody className="bg-white divide-y divide-sky-100">
                            {glParsedFileEntries.map((entry, index) => (
                              <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-sky-50/70'}>
                                <td className="px-2 py-1.5 whitespace-nowrap">
                                  <input type="number" value={entry.general_ledger_code}
                                         onChange={(e) => handleParsedEntryChange(index, 'general_ledger_code', e.target.value)}
                                         className="w-full p-1.5 border border-sky-200 rounded-md text-sm focus:ring-sky-500 focus:border-sky-500" />
                                </td>
                                <td className="px-2 py-1.5 whitespace-nowrap">
                                  <input type="text" value={entry.account_name}
                                         onChange={(e) => handleParsedEntryChange(index, 'account_name', e.target.value)}
                                         className="w-full p-1.5 border border-sky-200 rounded-md text-sm focus:ring-sky-500 focus:border-sky-500" />
                                </td>
                                <td className="px-2 py-1.5 whitespace-nowrap">
                                  <input type="text" value={entry.category}
                                         onChange={(e) => handleParsedEntryChange(index, 'category', e.target.value)}
                                         className="w-full p-1.5 border border-sky-200 rounded-md text-sm focus:ring-sky-500 focus:border-sky-500" />
                                </td>
                              </tr>
                            ))}
                  ...
                  <div className="flex justify-end mt-auto pt-4 border-t border-gray-200">
                    <button
                      type="button" onClick={submitGLParsedFileEntries} disabled={isGLLoading || glParsedFileEntries.length === 0}
                      className={`px-6 py-3 rounded-lg flex items-center text-sm ${isGLLoading || glParsedFileEntries.length === 0 ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 text-white'}`} >
                      {isGLLoading ? (<><TailSpin color="#FFFFFF" height={18} width={18} className="mr-2" /> Uploading...</>) : (<><FiUpload className="mr-2" /> Upload {glParsedFileEntries.length} {glParsedFileEntries.length === 1 ? 'Entry' : 'Entries'}</>)}
                    </button>
                  </div>
                </div>
            )}
...
{glActiveTab === 'history' && (
  <div>
    <div className='flex flex-col lg:flex-row items-start gap-6 mb-6'>
      {/* Illustration - Takes up some space on the left */}
      <div className="w-full lg:w-auto flex justify-center lg:justify-start flex-shrink-0">
          <img 
            src={glUploadIllustration} 
            className={glIllustrationCommonStyle + " max-w-[150px] sm:max-w-[180px]"} // Consistent smaller size
            alt="History illustration" 
          />
      </div>

      {/* Calendar - Central Element */}
      <div className="w-full max-w-xs sm:max-w-sm flex-shrink-0 bg-sky-50 rounded-xl border border-sky-300 shadow-xl relative overflow-hidden">
        <DatePicker
          selected={glSelectedDate}
          onChange={handleGLDateChange}
          inline
          renderCustomHeader={(props) => <CustomCalendarHeader {...props} />}
          calendarClassName="text-xs react-datepicker-small border-0 shadow-none !w-full !bg-transparent"
          wrapperClassName="!w-full"
        />
      </div>

      {/* Button - To the right of the calendar */}
      <div className="w-full lg:w-auto flex justify-center lg:justify-start flex-shrink-0 mt-4 lg:mt-0">
        <button
          onClick={fetchGLHistoryForSelectedDate}
          disabled={isFetchingGLHistory}
          className={`w-full max-w-xs sm:max-w-[280px] px-5 py-3 text-sm font-semibold text-white rounded-lg shadow-md 
                      flex items-center justify-center transition-all duration-200 ease-in-out group
                      focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-sky-50
                      ${isFetchingGLHistory 
                          ? 'bg-sky-400 cursor-not-allowed' 
                          : 'bg-sky-600 hover:bg-sky-700 hover:shadow-lg transform hover:-translate-y-0.5'
                      }`}
        >
          {isFetchingGLHistory ? (
            <>
              <TailSpin color="#FFFFFF" height={20} width={20} wrapperClass="mr-2" /> 
              Fetching...
            </>
          ) : (
            <>
              <FiCalendar className="mr-2 h-5 w-5 transition-transform duration-200 ease-in-out group-hover:rotate-[-5deg]" /> 
              Fetch for {glSelectedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
            </>
          )}
        </button>
      </div>
    </div>
    <p className="text-sm text-gray-600 mb-6 -mt-4 text-center px-4">
      Select a date using the calendar and click the button to retrieve the General Ledger activity history for that specific day.
    </p>


    {/* ... rest of the history data display logic (loading, no data, table) ... */}
    {isFetchingGLHistory && !glHistoryData && (
        <div className="text-center py-8 mt-6">
            <TailSpin color="#0EA5E9" height={40} width={40} />
            <p className="text-sky-600 mt-2">Loading history for {glHistoryFetchDate?.toLocaleDateString()}...</p>
        </div>
    )}

    {glHistoryFetchDate && !isFetchingGLHistory && glHistoryData && glHistoryData.length === 0 && (
        <div className="p-6 text-center bg-yellow-50 border border-yellow-300 rounded-lg mt-6">
            <FiInfo className="w-10 h-10 text-yellow-500 mx-auto mb-3" />
            <p className="text-lg font-medium text-yellow-700">No History Found</p>
            <p className="text-sm text-yellow-600">There are no GL activities recorded for {glHistoryFetchDate.toLocaleDateString()}.</p>
        </div>
    )}
    ...
    {glHistoryFetchDate && !isFetchingGLHistory && glHistoryData && glHistoryData.length > 0 && (
      <div className="mt-2"> {/* Reduced margin-top */}
        <h4 className="text-lg font-semibold text-gray-700 mb-3">
          History for: {glHistoryFetchDate.toLocaleDateString()} ({glHistoryData.length} {glHistoryData.length === 1 ? "record" : "records"})
        </h4>
        <div className="overflow-x-auto rounded-lg border border-sky-100 max-h-[calc(100vh-480px)]"> {/* Adjusted max-h further */}
            <table className="min-w-full divide-y divide-sky-100">
                <thead className="bg-sky-50 sticky top-0 z-10">
                    <tr>
                        <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">GL Code</th>
                        <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">Account Name</th>
                        <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">Amount</th>
                        <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">Type/Source</th>
                        <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase">Timestamp</th>
                    </tr>
                </thead>
                <tbody className="bg-white divide-y divide-sky-100">
                    {glHistoryData.map(item => (
                        <tr key={item.id} className="hover:bg-sky-50/70">
                            <td className="px-3 py-2 whitespace-nowrap text-sm">{item.general_ledger_code}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm">{item.account_name}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm">{typeof item.amount === 'number' ? item.amount.toFixed(2) : item.amount}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm">{item.type_of_entry}</td>
                            <td className="px-3 py-2 whitespace-nowrap text-sm">{formatDate(item.created_at)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
                    ...
                    <table className="min-w-full divide-y divide-sky-100">
                      <thead className="bg-sky-50 sticky top-0 z-10">
                        <tr>
                          ...
                          <th className="px-3 py-2.5 text-left text-xs font-medium text-sky-600 uppercase tracking-wider">ID</th>
                      ...
                      <tbody className="bg-white divide-y divide-sky-100">
                        {allGLCodesData.map((code, index) => (
                          <tr key={code.id || index} className={index % 2 === 0 ? 'bg-white' : 'bg-sky-50/70 hover:bg-sky-100'}>
        ...
        {activeTab === 'dashboard' && ( <div> <h3 className="text-2xl font-semibold text-sky-800 mb-1">Dashboard & UI Customization</h3> <p className="text-gray-600 mb-6 text-sm">Personalize your dashboard layout, color scheme, and display preferences for an enhanced user experience.</p> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> <div className="border border-gray-200 rounded-lg p-6 bg-gray-50"> <h4 className="font-semibold text-lg text-sky-700 mb-4">Color Settings</h4> <div className="space-y-4"> <div> <label className="block text-sm font-medium text-gray-600 mb-1">Primary Color</label> <div className="flex items-center"> <input type="color" value={colorSettings.primaryColor} onChange={(e) => handleColorChange(e, 'primaryColor')} className="w-10 h-10 rounded-md cursor-pointer border-2 border-gray-300 shadow-sm"/> <span className="ml-3 text-sm text-gray-700 p-2 bg-white border border-gray-200 rounded-md">{colorSettings.primaryColor}</span> </div> </div> <div> <label className="block text-sm font-medium text-gray-600 mb-1">Text Color</label> <div className="flex items-center"> <input type="color" value={colorSettings.textColor} onChange={(e) => handleColorChange(e, 'textColor')} className="w-10 h-10 rounded-md cursor-pointer border-2 border-gray-300 shadow-sm"/> <span className="ml-3 text-sm text-gray-700 p-2 bg-white border border-gray-200 rounded-md">{colorSettings.textColor}</span> </div> </div> <div> <label className="block text-sm font-medium text-gray-600 mb-1">Background Color</label> <div className="flex items-center"> <input type="color" value={colorSettings.backgroundColor} onChange={(e) => handleColorChange(e, 'backgroundColor')} className="w-10 h-10 rounded-md cursor-pointer border-2 border-gray-300 shadow-sm"/> <span className="ml-3 text-sm text-gray-700 p-2 bg-white border border-gray-200 rounded-md">{colorSettings.backgroundColor}</span> </div> </div> </div> </div> <div className="border border-gray-200 rounded-lg p-6 bg-gray-50"> <h4 className="font-semibold text-lg text-sky-700 mb-4">Font Settings</h4> <div className="space-y-4"> <div> <label className="block text-sm font-medium text-gray-600 mb-1">Font Size</label> <select className="w-full border border-gray-300 rounded-md p-2.5 text-sm shadow-sm focus:ring-sky-500 focus:border-sky-500" value={colorSettings.fontSize} onChange={(e) => setColorSettings({...colorSettings, fontSize: e.target.value})}> <option value="12px">Small (12px)</option> <option value="14px">Medium (14px)</option> <option value="16px">Large (16px)</option> </select> </div> <div> <label className="block text-sm font-medium text-gray-600 mb-1">Font Family</label> <select className="w-full border border-gray-300 rounded-md p-2.5 text-sm shadow-sm focus:ring-sky-500 focus:border-sky-500" value={colorSettings.fontFamily} onChange={(e) => setColorSettings({...colorSettings, fontFamily: e.target.value})}> <option value="Inter">Inter</option> <option value="Roboto">Roboto</option> <option value="Open Sans">Open Sans</option> <option value="Montserrat">Montserrat</option> </select> </div> </div> </div> </div> <div className="mt-8 flex justify-end"> <button className="bg-sky-600 text-white px-6 py-2.5 rounded-lg hover:bg-sky-700 transition-colors font-medium text-sm shadow-md" onClick={handleSubmitSettings}>Save UI Settings</button> </div> </div> )}
        {activeTab === 'reports' && ( <div> <h3 className="text-2xl font-semibold text-sky-800 mb-1">Report & Workflow Automation</h3> <p className="text-gray-600 mb-6 text-sm">Configure automated report generation and financial workflows for effortless financial management.</p> <div className="space-y-6"> <div className="border border-gray-200 rounded-lg p-6 bg-gray-50"> <h4 className="font-semibold text-lg text-sky-700 mb-3">Scheduled Reports</h4> <p className="text-sm text-gray-500 mb-4">Set up automatic report generation and distribution to stakeholders.</p> <button className="flex items-center text-sm bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition-colors font-medium shadow-md"><FiPlus className="mr-2" /> Add New Schedule</button> </div> <div className="border border-gray-200 rounded-lg p-6 bg-gray-50"> <h4 className="font-semibold text-lg text-sky-700 mb-3">Workflow Rules</h4> <p className="text-sm text-gray-500 mb-4">Create rules to automate financial processes, such as approval routing or data validation checks.</p> <button className="flex items-center text-sm bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition-colors font-medium shadow-md"><FiPlus className="mr-2" /> Create New Rule</button> </div> </div> </div> )}
        {activeTab === 'alerts' && ( <div> <h3 className="text-2xl font-semibold text-sky-800 mb-1">Alerts & Notification Preferences</h3> <p className="text-gray-600 mb-6 text-sm">Manage your email notification settings to stay informed and proactive.</p> <div className="border border-gray-200 rounded-lg p-6 max-w-3xl mx-auto bg-gray-50"> <h4 className="font-semibold text-lg text-sky-700 mb-5">Email Notifications</h4> <div className="space-y-5"> {Object.entries(emailNotifications).map(([key, value]) => ( <label key={key} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow"> <div> <span className="text-gray-800 font-medium capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</span> <p className="text-xs text-gray-500 mt-0.5"> {key === 'transactionAlerts' && 'Receive emails for important transactions like large payments or unusual activity.'} {key === 'reportReady' && 'Get notified when scheduled reports are generated and ready for review.'} {key === 'systemUpdates' && 'Receive important system updates, maintenance notifications, and new feature announcements.'} {key === 'weeklySummary' && 'Get a weekly summary of your key financial activities and metrics.'} </p> </div> <input type="checkbox" className="form-checkbox h-5 w-5 text-sky-600 rounded border-gray-300 focus:ring-sky-500 transition duration-150 ease-in-out" checked={value} onChange={() => handleEmailNotificationChange(key)}/> </label> ))} </div> <div className="mt-8 flex justify-end"> <button className="bg-sky-600 text-white px-6 py-2.5 rounded-lg hover:bg-sky-700 transition-colors font-medium text-sm shadow-md">Save Notification Settings</button> </div> </div> </div> )}
        {activeTab === 'system' && ( <div> <h3 className="text-2xl font-semibold text-sky-800 mb-1">System & Data Preferences</h3> <p className="text-gray-600 mb-6 text-sm">Configure platform settings, data management options, and ensure compliance.</p> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> <div className="border border-gray-200 rounded-lg p-6 bg-gray-50"> <h4 className="font-semibold text-lg text-sky-700 mb-3">Data Backup & Retention</h4> <p className="text-sm text-gray-500 mb-4">Configure automatic data backup schedules and define data retention policies.</p> <button className="flex items-center text-sm bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition-colors font-medium shadow-md"><FiSettings className="mr-2" /> Backup Settings</button> </div> <div className="border border-gray-200 rounded-lg p-6 bg-gray-50"> <h4 className="font-semibold text-lg text-sky-700 mb-3">Data Export & API Access</h4> <p className="text-sm text-gray-500 mb-4">Set up data export formats, schedules, and manage API access keys for integrations.</p> <button className="flex items-center text-sm bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition-colors font-medium shadow-md"><FiDatabase className="mr-2" /> Export & API Settings</button> </div> </div> </div> )}
        {activeTab === 'roles' && ( <div> <h3 className="text-2xl font-semibold text-sky-800 mb-1">Role-Based Feature Access</h3> <p className="text-gray-600 mb-6 text-sm">Manage team permissions and feature access controls to ensure appropriate data visibility and operational security.</p> <div className="overflow-x-auto bg-gray-50 p-1 rounded-lg border border-gray-200"> <table className="min-w-full divide-y divide-gray-200"> <thead className="bg-gray-100"><tr> <th className="px-6 py-3 text-left text-xs font-semibold text-sky-700 uppercase tracking-wider">Role</th> <th className="px-6 py-3 text-left text-xs font-semibold text-sky-700 uppercase tracking-wider">Dashboard Access</th> <th className="px-6 py-3 text-left text-xs font-semibold text-sky-700 uppercase tracking-wider">Reporting</th> <th className="px-6 py-3 text-left text-xs font-semibold text-sky-700 uppercase tracking-wider">Settings Access</th><th className="px-6 py-3 text-left text-xs font-semibold text-sky-700 uppercase tracking-wider">Actions</th></tr></thead> <tbody className="bg-white divide-y divide-gray-200"> {/* Sample Data - Replace with dynamic data */} <tr><td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Administrator</td><td className="px-6 py-4 text-sm text-gray-500">Full Control</td><td className="px-6 py-4 text-sm text-gray-500">Create & View All</td><td className="px-6 py-4 text-sm text-gray-500">Full Control</td><td className="px-6 py-4"><button className="text-sky-600 hover:text-sky-800 text-sm"><FiEdit2/></button></td></tr> <tr><td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Finance Manager</td><td className="px-6 py-4 text-sm text-gray-500">View & Edit</td><td className="px-6 py-4 text-sm text-gray-500">Create & View Departmental</td><td className="px-6 py-4 text-sm text-gray-500">View Only</td><td className="px-6 py-4"><button className="text-sky-600 hover:text-sky-800 text-sm"><FiEdit2/></button></td></tr> <tr><td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Department Head</td><td className="px-6 py-4 text-sm text-gray-500">View Departmental</td><td className="px-6 py-4 text-sm text-gray-500">View Departmental Only</td><td className="px-6 py-4 text-sm text-gray-500">None</td><td className="px-6 py-4"><button className="text-sky-600 hover:text-sky-800 text-sm"><FiEdit2/></button></td></tr> </tbody> </table> </div> <div className="mt-6 flex justify-start"> <button className="flex items-center text-sm bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 transition-colors font-medium shadow-md"><FiPlus className="mr-2" /> Add New Role</button> </div> </div> )}
