...
const ProjectCostingReport = () => {
  ...
  // Sample data for Project Costing report
  const projectCostingData = {
    tableData: [
      { id: 1, project: "Q1 Marketing Campaign", department: "Marketing", budget: 75000, actual: 72000, completion: 100, status: "Completed" },
      { id: 2, project: "ERP System Upgrade", department: "IT", budget: 150000, actual: 165000, completion: 95, status: "In Progress" },
      { id: 3, project: "New Feature Development", department: "R&D", budget: 120000, actual: 110000, completion: 100, status: "Completed" },
      { id: 4, project: "Sales Team Training", department: "Sales", budget: 30000, actual: 35000, completion: 100, status: "Completed" },
      { id: 5, project: "Website Redesign", department: "Marketing", budget: 50000, actual: 25000, completion: 50, status: "In Progress" },
      { id: 6, project: "New Office Build-out", department: "Operations", budget: 250000, actual: 150000, completion: 60, status: "In Progress" },
    ],
    metrics: {
      totalBudget: 675000,
      totalActual: 557000,
      totalVariance: 118000,
      onBudgetProjects: 4,
      overBudgetProjects: 2,
      insights: [
        "Overall project portfolio is currently $118k under budget, driven by the new office build-out.",
        "Sales Team Training and ERP Upgrade are over budget, requiring review.",
        "AI suggests reallocating the $10k savings from R&D to the Website Redesign to accelerate completion."
      ]
    }
  };
  ...
  const handleDrillDown = (rowData) => {
    ...
    setDrillDownData({
      ...
      insights: [
        ...
        "Resource allocation appears to be the primary driver of the current cost.",
        "Contact Project Manager: Jane Doe (jane.doe@company.com)"
      ]
    });
  };
  ...
  const getDrillDownInsight = (rowData) => {
    const variance = rowData.budget - rowData.actual;
    if (variance < 0) {
      return `Project is currently ${formatCurrency(Math.abs(variance))} over budget. A review of spending is recommended.`;
    } else {
      return `Project is ${formatCurrency(variance)} under budget, indicating efficient cost management.`;
    }
  };

  const getCSVData = () => {
    return projectCostingData.tableData.map(row => ({
        Project: row.project,
    ...
    return (
      <div className="overflow-y-auto bg-white/50 rounded-lg border border-sky-100 max-h-[30rem]">
        <table className="w-full text-xs">
          ...
          <tbody>
            {projectCostingData.tableData.map((row, index) => {
                    ...
                    >
                        ...
                        <td className="px-4 py-2">
                           <div className="w-full bg-gray-200 rounded-full h-2.5">
                             <div className={`h-2.5 rounded-full ${row.completion > 90 ? 'bg-green-500' : 'bg-blue-500'}`} style={{width: `${row.completion}%`}}></div>
                           </div>
                        </td>
             ...
             <tr className="bg-gray-100 font-semibold sticky bottom-0">
              ...
              <td className="px-4 py-2 text-right">{formatCurrency(projectCostingData.metrics.totalBudget)}</td>
              <td className="px-4 py-2 text-right">{formatCurrency(projectCostingData.metrics.totalActual)}</td>
              <td className={`px-4 py-2 text-right ${projectCostingData.metrics.totalVariance >= 0 ? "text-green-600" : "text-red-600"}`}>
                {formatCurrency(projectCostingData.metrics.totalVariance)}
              </td>
              <td colSpan="2"></td>
            </tr>
          </tbody>
  ...
  const renderKeyMetrics = () => {
    const { metrics } = projectCostingData;
    return (
      <div className="space-y-3">
        ...
        <ul className="text-sm text-black space-y-2">
          ...
          <li className="flex justify-between items-center">
            ...
            <span className="font-medium text-red-600">{metrics.overBudgetProjects}</span>
          </li>
        </ul>
        ...
        {metrics.insights && (
          <div className="mt-4">
            <h4 className="text-xs font-semibold text-sky-900 mb-2 flex items-center">
              <FiAlertCircle className="mr-1" /> Project Insights
            </h4>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-sky-50 rounded-lg p-4">
            <h4 className="text-sm font-medium text-sky-800 mb-3">Project Details</h4>
  ...
  const handleSendAIQuery = () => {
    if (aiInput.trim()) {
      const response = `AI analysis for Project Costing: ${aiInput} (This is a mock response)`;
  ...
  useEffect(() => {
    if (projectCostingData.metrics.insights?.length > 1) {
      const interval = setInterval(() => {
        setActiveInsight((prev) => (prev + 1) % projectCostingData.metrics.insights.length);
      }, 5000);
      return () => clearInterval(interval);
    }
  }, [projectCostingData.metrics.insights]);
    ...
    <div className="min-h-screen bg-sky-50 p-6">
      <div className="bg-gradient-to-r from-[#004a80] to-[#cfe6f7] p-4 rounded-lg mb-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-lg font-bold text-white">Project Costing Report</h1>
            <p className="text-sky-100 text-xs">Monitors expenses and profitability of specific projects.</p>
          </div>
        ...
        <ol className="inline-flex items-center space-x-1 md:space-x-2">
          ...
          <li aria-current="page">
            <div className="flex items-center">
              ...
              <span className="ms-1 text-sm font-medium text-gray-500 md:ms-2">Project Costing Report</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-lg font-semibold text-sky-900 flex items-center gap-2">
                {drillDownData ? drillDownData.title : "Project Costing Summary"}
              </h2>
              <p className="text-sky-600 text-sm mb-2">
                {drillDownData ? "Detailed view of project costs" : "Budget vs. Actual spend across all projects"}
              </p>
            </div>
        ...
        {!drillDownData && (
          <div className="mb-4 flex flex-wrap gap-2 justify-end items-center">
            ...
            <CSVLink data={getCSVData()} filename={"project-costing-report.csv"} className="flex items-center px-3 py-2 bg-sky-100 text-sky-800 rounded-lg text-xs hover:bg-sky-200">
            ...
            <div className="relative">
              ...
              {showAIDropdown && (
                  <motion.div ref={aiChatbotRef} className="absolute z-50 mt-2 right-0 w-80 bg-white/90 backdrop-blur-md rounded-lg shadow-lg border border-sky-200 p-2" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                    <h1 className="text-sm font-semibold text-sky-900 mb-2">Ask about Project Costing</h1>
...
export default ProjectCostingReport;