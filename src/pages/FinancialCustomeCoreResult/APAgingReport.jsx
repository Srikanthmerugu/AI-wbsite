...
const APAgingReport = () => {
  ...
  // Sample data for AP Aging report
  const apAgingData = {
    tableData: [
      { vendor: "Office Supplies Inc", current: 8000, days30: 3000, days60: 1500, days90: 1000, over90: 500, total: 14000 },
      { vendor: "Tech Solutions Ltd", current: 5000, days30: 2000, days60: 1000, days90: 500, over90: 0, total: 8500 },
      { vendor: "Utilities Corp", current: 12000, days30: 0, days60: 0, days90: 0, over90: 0, total: 12000 },
      { vendor: "Marketing Partners", current: 4000, days30: 1500, days60: 800, days90: 400, over90: 300, total: 7000 },
      { vendor: "Legal Services", current: 6000, days30: 1000, days60: 500, days90: 200, over90: 100, total: 7800 },
    ],
    summary: {
      current: 35000,
      days30: 7500,
      ...
      dpo: 40,
      insights: [
        "90+ days overdue payables represent only 1.8% of total payables",
        "Current payables account for 71% of total AP",
        "Vendor relationships appear healthy with minimal overdue amounts"
      ]
    }
  };
  ...
  const handleDrillDown = (rowData) => {
    setDrillDownData({
      title: `${rowData.vendor} AP Details`,
      data: [
      ...
      insights: [
        getVendorInsight(rowData),
        "Payment terms: Net 30 days",
        "Contact: Sarah Johnson (s.johnson@vendor.com) | Phone: (555) 987-6543"
      ]
    });
  };
  ...
  const getVendorInsight = (rowData) => {
    ...
    if (percentOverdue > 30) {
      return `High overdue amount (${percentOverdue.toFixed(1)}% of total) - review payment terms`;
    } else if (percentOverdue > 15) {
      return `Moderate overdue amount (${percentOverdue.toFixed(1)}% of total) - schedule payment`;
    } else if (overdue > 0) {
    ...
    return (
      <div className="overflow-y-auto bg-white/50 rounded-lg border border-sky-100">
        <table className="w-full text-xs">
          ...
          <tbody>
            {apAgingData.tableData.map((row, index) => (
              <tr 
                key={index} 
                className="border-b border-sky-200 hover:bg-sky-50 cursor-pointer"
                onClick={() => handleDrillDown(row)}
              >
                <td className="px-4 py-2 font-medium">{row.vendor}</td>
            ...
            <tr className="bg-gray-100 font-semibold">
              ...
              <td className="px-4 py-2 text-right text-green-600">{formatCurrency(apAgingData.summary.current)}</td>
              <td className="px-4 py-2 text-right text-yellow-600">{formatCurrency(apAgingData.summary.days30)}</td>
              <td className="px-4 py-2 text-right text-orange-600">{formatCurrency(apAgingData.summary.days60)}</td>
              <td className="px-4 py-2 text-right text-red-600">{formatCurrency(apAgingData.summary.days90)}</td>
              <td className="px-4 py-2 text-right text-red-700">{formatCurrency(apAgingData.summary.over90)}</td>
              <td className="px-4 py-2 text-right">{formatCurrency(apAgingData.summary.total)}</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <ul className="text-sm text-black space-y-2">
          <li className="flex justify-between items-center">
            ...
            <span className="font-medium text-sky-800">
              {formatCurrency(apAgingData.summary.total)}
            </span>
          </li>
          <li className="flex justify-between items-center">
            <span>Overdue ({">"}30 days):</span>
            <span className="font-medium text-red-600">
              {formatCurrency(apAgingData.summary.days30 + apAgingData.summary.days60 + apAgingData.summary.days90 + apAgingData.summary.over90)}
            </span>
          </li>
          <li className="flex justify-between items-center">
            <span>90+ Days Overdue:</span>
            <span className="font-medium text-red-700">
              {formatCurrency(apAgingData.summary.over90)} ({apAgingData.summary.percentOver90}%)
            </span>
          </li>
          <li className="flex justify-between items-center">
            <span>Days Payable Outstanding (DPO):</span>
            <span className="font-medium text-sky-800">
              {apAgingData.summary.dpo} days
            </span>
          </li>
        </ul>

        {apAgingData.summary.insights && (
          <div className="mt-4">
            <h4 className="text-xs font-semibold text-sky-900 mb-2 flex items-center">
              <FiAlertCircle className="mr-1" /> Payables Insights
            </h4>
            <AnimatePresence mode="wait">
              ...
              >
                {apAgingData.summary.insights[activeInsight]}
              </motion.div>
            </AnimatePresence>
          </div>
        )}
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-sky-50 rounded-lg p-4">
            <h4 className="text-sm font-medium text-sky-800 mb-3">Vendor Insights</h4>
  ...
  const handleSendAIQuery = () => {
    if (aiInput.trim()) {
      const response = `AI analysis for AP Aging: ${aiInput} (This is a mock response showing how AI insights would appear)`;
  ...
  useEffect(() => {
    if (apAgingData.summary.insights?.length > 1) {
      const interval = setInterval(() => {
        setActiveInsight((prev) => (prev + 1) % apAgingData.summary.insights.length);
      }, 5000);
    ...
    <div className="min-h-screen bg-sky-50 p-6">
      <div className="bg-gradient-to-r from-[#004a80] to-[#cfe6f7] p-4 rounded-lg mb-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-lg font-bold text-white">AP Aging Report</h1>
            <p className="text-sky-100 text-xs">
              Outstanding payables by aging period
            </p>
          </div>
          <button
        ...
        <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          ...
          <li aria-current="page">
            <div className="flex items-center">
              ...
              <span className="ms-1 text-sm font-medium text-gray-500 md:ms-2">AP Aging Report</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-lg font-semibold text-sky-900 flex items-center gap-2">
                {drillDownData ? drillDownData.title : "Accounts Payable Aging Report"}
              </h2>
              <p className="text-sky-600 text-sm mb-2">
                {drillDownData ? "Detailed view of vendor payables" : "Outstanding bills categorized by days overdue"}
              </p>
            </div>
        ...
        {!drillDownData && (
          <div className="mb-4 flex flex-wrap gap-2 justify-end items-center">
            ...
            <div className="flex gap-2">
              <CSVLink
                data={apAgingData.tableData}
                filename={"ap-aging-report.csv"}
            ...
            <div className="bg-sky-50/50 p-4 rounded-lg border border-sky-200">
              ...
              <div className="text-xs text-sky-700 space-y-3">
                <div className="flex items-start">
                  ...
                  <div>
                    <p className="font-medium">Current Payables</p>
                    <p className="text-black">
                      {formatCurrency(apAgingData.summary.current)} ({((apAgingData.summary.current / apAgingData.summary.total) * 100).toFixed(1)}% of total)
                    </p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-black">
                      {formatCurrency(apAgingData.summary.days30)} ({((apAgingData.summary.days30 / apAgingData.summary.total) * 100).toFixed(1)}% of total)
                    </p>
                  </div>
                </div>
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-black">
                      {formatCurrency(apAgingData.summary.days60 + apAgingData.summary.days90 + apAgingData.summary.over90)} ({((apAgingData.summary.days60 + apAgingData.summary.days90 + apAgingData.summary.over90) / apAgingData.summary.total * 100).toFixed(1)}% of total)
                    </p>
                  </div>
                </div>
              </div>
